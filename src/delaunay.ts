...
export class Delaunay {
	...
	private triangulate(): Triangle[] {
		...
		/* Create array with indicies sorted by vertices' x-position */
		...
		/* Incrementaly add each vertex to the mesh */
		for (let i = 0; i < indices.length; ++i) {
			...
			for (let j = 0; j < open.length; ++j) {
				...
				const dx: number = this.vertices[indices[i]].x - open[j].ccMiddle.x;
				...
				const dy: number = this.vertices[indices[i]].y - open[j].ccMiddle.y;
			...
			for (let j: number = 0; j < edges.length; ++j) {
				const b = edges[j].p2;
				const a = edges[j].p1;
				const triangle: Triangle = new Triangle(a, b, this.vertices[indices[i]]);
		...
		for (let i: number = closed.length; i--; ) {
			if (!this.supertri.isSharingVertexWith(closed[i])) {
				open.push(closed[i]);
			}
		}
		/* Yay, we're done! */
